...
public class TrueTypeFont {
	...
	public void drawChar(char c, double x, double y)
		...
		final Rectangle2D bounds = fontMetrics.getStringBounds(Character.toString(c), null);
		try {
			if(bounds != null && charArray.length > c)
				drawQuad((float)x, (float)y, (float) charArray[c].width, (float) charArray[c].height, charArray[c].storedX, charArray[c].storedY, (float) charArray[c].width, (float) charArray[c].height);
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
	private void createSet( char[] customCharsArray ) {
		  ...
		  try {
		   ...
		   fontTextureID = TextureUtil.uploadTextureImageAllocate(TextureUtil.glGenTextures(),
		     imgTemp, false, false);

...
public class TrueTypeFont {
	...
	public void renderString(String s, float f, float g, int color, boolean flag) {
		...
		float red = getRGBA(color)[0];
		float green = getRGBA(color)[1];
		float blue = getRGBA(color)[2];
		float alpha = flag ? 1.0F : getRGBA(color)[3];
		...
		for(int i = 0; i < size; i++) {
			...
			if((c == 167) && (i + 1 < s.length())) {
				...
				if(j < 16) {
					try {
						int l = Distinct.getInstance().getMinecraft().fontRenderer.colorCode[j];
	...
	public void drawStringWithShadow(String s, float x, float f, int color) {
		x/=2;
		int l = color & 0x0;
		int shade = (color & 16579836) >> 2 | color & -16777216;
	...
	private void drawChar(char c, float f, float g) throws ArrayIndexOutOfBoundsException {
		Gui gui =  Distinct.getInstance().getMinecraft().ingameGUI;
